...
BASE_EMAIL_BODY = (
    'Hello!\n\n'
    'We draw your attention to the established fact of violation of the company\'s Information Security Policy: in {confluence_task} placement of restricted access information on a non-corporate resource {confluence_secret} in open access was detected, which poses a threat to the confidentiality and integrity of the company\'s information assets.\n\n'
    ...
    '1. for what purpose you created and/or used the document in {confluence_secret};\n'
    ...
    '4. whether you have familiarized yourself with the company\'s Information Security Policy.\n\n'
    'We strongly recommend refraining from further violations of information processing and storage rules, and immediately ceasing any such actions. Please confirm receipt of this notification and take necessary measures to eliminate the violations (transfer information from non-corporate cloud resources to corporate cloud resources, delete the document in {confluence_secret}).\n\n'
    'Consequences of non-compliance with these requirements will be considered by the company management and may lead to disciplinary sanctions up to termination of employment contract.\n\n'
...
def fetch_user_info(client):
                ...
                "must": [
                    {
                        "bool": {
                            "should": [
                                {"match_phrase": {"rule.id": "100105"}} # Change rule id if necessary
                            ],
    ...
    for hit in resp.get('hits', {}).get('hits', []):
        try:
            ...
            confluence_creator = source["confluence_creator"]
            confluence_task = source.get("confluence_task", "unknown")
            confluence_secret = source.get("confluence_secret", "unknown")

            if confluence_creator and isinstance(confluence_creator, str):
                # Normalize username
                u = confluence_creator.strip()
                if "\\" in u:
                    u = u.split("\\", 1)[1]
                if u.endswith("$"):
                    continue
                ...
                if u not in user_info:
                    user_info[u] = {
                        ...
                        'confluence_secret': confluence_secret
                    }
        except KeyError:
            continue
    ...
    for username, info in sorted(user_info.items()):
        ...
        try:
            ...
            email_body = BASE_EMAIL_BODY.format(
                confluence_task=info['confluence_task'],
                confluence_secret=info['confluence_secret']
            )
            send_email(email, EMAIL_SUBJECT, email_body)
            print(f"Sent: {email} (Task: {info['confluence_task']}, secret: {info['confluence_secret']})")
